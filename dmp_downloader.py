...
def download_dmp_data(current_month: str, current_year: int) -> Workbook:
    """Function to return latest DMP data file for processing"""

    url: str = f"https://www.bankofengland.co.uk/-/media/boe/files/decision-maker-panel-survey/{current_year}/monthly-dmp-data-for-website-{current_month}-{current_year}.xlsx"
    response: requests.Response = requests.get(url)

    if response.status_code == 200:
        output: TextIO = open(f"dmp_{current_month}_{current_year}.xlsx", "wb")
        output.write(response.content)
        output.close()

    wb: Workbook = load_workbook(f"dmp_{current_month}_{current_year}.xlsx")

    return wb

def months_diff(date_1: datetime.date, date_2: datetime.date) -> int:
...
def prepare_xlsx_file(
    ...
    for ws_name in ws_names:
        ...
        ws.cell(row=row_number, column=1).value = "Month"
    ...
    wb.save(f"dmp_{current_month}_{current_year}.xlsx")
...
def save_csv_file(
    ...
    for ws_name in ws_names:
        ...
        output_file_path: str = f"{formatted_name}_{current_month}_{current_year}.csv"
...
def download_and_process_dmp_data(
    ...
    wb: Workbook = download_dmp_data(current_month, current_year)
    starting_row_dict: dict[str, int] = prepare_xlsx_file(wb, ws_names, current_month, current_year)
    output_file_paths: list[str] = save_csv_file(wb, ws_names, current_month, current_year, starting_row_dict, column_lengths_dict)
